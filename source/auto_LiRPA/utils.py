...
# from collectionsAbc import defaultdict, Sequence, namedtuple
...
eyeC = collectionsAbc.namedtuple('eyeC', 'shape device')
OneHotC = collectionsAbc.namedtuple('OneHotC', 'shape device index coeffs')
...
def namedtuple_with_defaults(name, attr, defaults):
    ...
    if sys.version_info.major >= 7:
        return collectionsAbc.namedtuple(name, attr, defaults=defaults)
    else:
        # The defaults argument is not available in Python < 3.7
        t = collectionsAbc.namedtuple(name, attr)
...
class MultiAverageMeter(object):
    ...
    def __init__(self):
        self.reset()
    def reset(self):
        self.sum_meter = collectionsAbc.defaultdict(float)
        self.lasts = collectionsAbc.defaultdict(float)
        self.counts_meter = collectionsAbc.defaultdict(int)
...
class MultiTimer(object):
    ...
    def __init__(self):
        self.reset()
    def reset(self):
        self.timer_starts = collectionsAbc.defaultdict(float)
        self.timer_total = collectionsAbc.defaultdict(float)
    def start(self, key):
...
def recursive_map (seq, func):
    for item in seq:
        if isinstance(item, collectionsAbc.Sequence):
